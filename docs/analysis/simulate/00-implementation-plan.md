...
#### **Stage 1: 專案初始化和 Proto 編譯** ⏱️ 2 小時

**目標**: 建立 C# 專案結構並編譯 Proto 文件

**任務清單**:
- [ ] 建立解決方案 `YogoCloudSimulate.sln`
- [ ] 建立專案 `Common`、`StationSimulator`、`RobotSimulator`
- [ ] 建立測試專案 `Common.Tests`、`Integration.Tests`
- [ ] 安裝 NuGet 套件（Grpc.Net.Client、Google.Protobuf、Grpc.Tools）
- [ ] 複製 `jarvis_iot.proto` 到 `Common/Protos/`
- [ ] 配置 Proto 編譯（.csproj 設定）
- [ ] 編譯並驗證生成的 C# 類別
...
#### **Stage 2: gRPC 基礎連接** ⏱️ 3 小時

**目標**: 實作基礎的 gRPC TLS 連接

**任務清單**:
- [ ] 建立 `GrpcClientBase` 類別
- [ ] 實作 `ConnectAsync()` 方法（TLS 連接到 `robot-rpc.aurotek.com:443`）
- [ ] 實作連接狀態檢查
- [ ] 實作錯誤處理和重試機制
- [ ] 實作 `Dispose()` 資源清理
- [ ] 撰寫單元測試 `GrpcClientBaseTests.cs`
...
#### **Stage 3: RSRegisterUnit 註冊功能** ⏱️ 3 小時

**目標**: 實作設備註冊功能

**任務清單**:
- [ ] 建立 `UnitRegistration` 類別
- [ ] 實作 `RegisterUnitAsync()` 方法
- [ ] 處理註冊請求和響應
- [ ] 保存 `unit_uuid` 和 `unit_uid`
- [ ] 處理註冊錯誤（已註冊、參數錯誤等）
- [ ] 撰寫單元測試 `UnitRegistrationTests.cs`
...
#### **Stage 4: RSLoginUnit 登入功能** ⏱️ 3 小時

**目標**: 實作設備登入功能

**任務清單**:
- [ ] 建立 `UnitLogin` 類別
- [ ] 實作 `LoginUnitAsync()` 方法
- [ ] 處理登入請求和響應
- [ ] 保存 token
- [ ] 處理登入錯誤（未註冊、認證失敗等）
- [ ] 撰寫單元測試 `UnitLoginTests.cs`
...
#### **Stage 5: 任務狀態機** ⏱️ 3 小時

**目標**: 實作任務狀態機

**任務清單**:
- [ ] 定義 `TaskState` 枚舉
- [ ] 建立 `TaskStateMachine` 類別
- [ ] 實作狀態轉換邏輯
- [ ] 實作狀態轉換驗證
- [ ] 實作狀態變更事件
- [ ] 撰寫單元測試 `TaskStateMachineTests.cs`
...
#### **Stage 6: 本地消息總線（2.4G 模擬）** ⏱️ 4 小時

**目標**: 實作本地消息總線，模擬 2.4G 通信

**任務清單**:
- [ ] 建立 `LocalMessageBus` 類別
- [ ] 實作發布/訂閱機制
- [ ] 實作主題（Topic）管理
- [ ] 實作消息佇列
- [ ] 實作線程安全
- [ ] 撰寫單元測試 `LocalMessageBusTests.cs`
...
#### **Stage 7: StreamUnitControl 接收任務** ⏱️ 4 小時
...
#### **Stage 8: StreamUnitEvents 上報事件** ⏱️ 4 小時
...
1. ✅ **閱讀本文檔** - 完整理解實作計劃
2. 📅 **建立 Phase 1 規劃** - 詳細規劃前 6 個 Stage
3. 📅 **開始 Stage 1** - 專案初始化和 Proto 編譯
